...
public class QueueRoutin : Singleton<QueueRoutin>
{
    ...
    {
        if (LevelControl.Instance.TimeDay) return;
    ...
    {
        if (!(LevelControl.Instance.TimeDay))     //밤일경우만 선택가능
        {
            LOCK = !LOCK;
            if (LOCK)
            {
            ...
            if (!Settings.Instance.IsSOUNDClicked)
            {
    ...
    public IEnumerator StartRoutin()        //루틴상 전체 과정 
    {
        while (true)
        {
            ...
            yield return new WaitUntil(() =>
                rou.Count > 0 &&
                LevelControl.Instance.TimeDay &&
                LevelControl.Instance.ene > 10 &&
            ...
            image_fill = bar;
            PlayerCtrl.Instance.PlayerGet(target.name);
    ...
    {
        if (LevelControl.Instance.TimeDay) return; //Only excute at night
        ...
        var drag = inst.GetComponent<DragAbleUI>();
    ...
    public GameObject[] RandomBlock(string a)   //blue1 - green2 - yellow3 - red4
    {
        ...
        if (rn <= 8 && LevelControl.Instance.fat < 10)
        {
            return Lv4;
        }
        else if (rn <= 20 && LevelControl.Instance.fat < 25)
        {
            return Lv3;
        }
        else if (rn <= 50 && LevelControl.Instance.fat < 50)
        {
            return Lv2;
        }
        else // (50 <= rn && rn <= 100 && LevelControl.Instance.fat > 50)
        {
            return Lv1;
        }
    }
