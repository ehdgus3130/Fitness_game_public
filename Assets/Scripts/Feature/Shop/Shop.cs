...
public class Shop : Singleton<Shop>
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < TimeItem.Length; i++)
        {
            ...
            if (!newone.TryGetComponent<SellItem>(out var si))
            {
                Destroy(newone);
                Debug.LogError("[Shop] SellItem 컴포넌트가 없습니다.");
                return;
            }
            si.UseRandom(6);
    ...
    {
        if (!Settings.Instance.IsSoundEnabled) ShopSound.Play();
    ...
    public void OnClick_GiftBox()
    {
        ...
        {
            SellItem it = si.GetComponent<SellItem>();

            Already_Itm_get.SetActive(true);
            Already_Itm_get.GetComponent<AlreadyOwnedItemGet>().SetName(newone.name);
            Already_Itm_get.GetComponent<AlreadyOwnedItemGet>().itmCore.sprite = it.coreimg.sprite;
            for (int i = 0; i < qty; i++)
        ...
        else                        //신규: “획득” UI + 인벤토리에 지급
        {
            ...
            Itm_get.GetComponent<ItemGet>().SetName(newone.name);
    ...
    void OnClick_1HTimeSale(GameObject item)
    {
        ...
        var si = item.GetComponent<SellItem>();
        if (!si) { Debug.LogError("[Shop] SellItem 없음"); return; }

        int qty = 1;
        TMP_Text pri = item.GetComponent<SellItem>().priceText;
        var raw = pri != null ? (pri.text ?? "").Trim().TrimStart('x', 'X', ' ') : "1";
        if (!int.TryParse(raw, out qty)) qty = 1; //String to int
        ...
        string itemName, int qty, float unitCost, Button placeBtn, out float newExp)
    {
        ...
        {                 // 기존 판정 유지
            ...
            Itm_get.SetActive(true);
            Itm_get.GetComponent<ItemGet>().SetName(itemName);
    ...
    public void OnClick_RandomBox()
    {
        ...
        int qty = 1;
        TMP_Text pri = newone.GetComponent<SellItem>().priceText;
        var raw = pri != null ? (pri.text ?? "").Trim().TrimStart('x', 'X', ' ') : "1";
        if (!int.TryParse(raw, out qty)) qty = 1; //String to int
        ...
        {
            Already_Itm_get.SetActive(true);
            var Already = Already_Itm_get.GetComponent<AlreadyOwnedItemGet>();
    ...
    private IEnumerator ExecuteEveryTenMinutes()
    {
        ...
        while (true)
        {
            ...
            {
                0 => RoutineQueueManager.Instance.Lv1[UnityEngine.Random.Range(0, RoutineQueueManager.Instance.Lv1.Length)],
                1 => RoutineQueueManager.Instance.Lv2[UnityEngine.Random.Range(0, RoutineQueueManager.Instance.Lv2.Length)],
                2 => RoutineQueueManager.Instance.Lv3[UnityEngine.Random.Range(0, RoutineQueueManager.Instance.Lv3.Length)],
                _ => RoutineQueueManager.Instance.Lv4[UnityEngine.Random.Range(0, RoutineQueueManager.Instance.Lv4.Length)],
            };
    ...
    bool IfOverlapItem(GameObject item)
    {
        switch (int.Parse(DataManager.Instance.GetCurrentPlayerId()))
        {
            ...
            case 2:
                ItemData thing1 = EquipmentScreen.Instance.ItmSlot2.Find(x => x.name == item.name);
                if (thing1 != null) return true;
                else return false;
            case 3:
                ItemData thing2 = EquipmentScreen.Instance.ItmSlot3.Find(x => x.name == item.name);
