...
public class Item_Explain : Singleton<Item_Explain>
{
    [Header("UI")]
    public Image itmCore;
    public TMP_Text text_Item;
    public TMP_Text text_Item_explain;
    public Sprite[] YesBtnImg; //enable / disable
    public Transform[] EXPPos;
    public Sprite[] Bar = new Sprite[8];
    public Button ToEquipMent;
    private Image[][] EXPS = new Image[8][];
    private Dictionary<string, Sprite> _spriteByName;
    private Dictionary<string, Itm> _itemByName;
    ...
    void Awake()
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                if (!EXPPos[i].GetChild(j).TryGetComponent<Image>(out var img))
                    Debug.LogError($"Row {i} Col {j} has no Image.");
                EXPS[i][j] = img;
            }
        }

        //Add New Hash Inforamtion
        ...
        _itemByName = new Dictionary<string, Itm>(StringComparer.Ordinal);
    ...
    DataManager.Instance.Player_Info[0].Item3 == name;
        ...
        {
            ToEquipMent.GetComponent<Image>().sprite = YesBtnImg[0];
        }
    }
    public void SetName(string rawName)
    {
        ...
        {
            text_Item_explain.text = "";
            // 알 수 없는 경우 EXP 기본 채움
            for (int i = 2; i < 8; i++) ApplyBars(i, Level.All);
            return;
        }

        text_Item_explain.text = equip_.explain ?? "";
        ...
        if (ToEquipMent != null)
        {
            ...
            ToEquipMent.onClick.RemoveAllListeners();
            ToEquipMent.onClick.AddListener(() =>
            {
                EquipmentScreen.Instance.Equip_Item(equip_, true);
                EquipmentScreen.Instance.EquipSound.Play();
